...
void init_cfg(global_cfg *g)
{
    ...
    for (size_t i = 0; i < VOICE_COUNT; i++) {
        ...
        // v->lowpass.harmonics = V_LOWPASS_H;
...
void filter_mono_chunk(float *buf, size_t frame_count, filter_state *fstate)
{
...
void mix_chunk(frame *buf, size_t frame_count,
               float *input,
               voice_state *vstate)
{
...
void filter_amp_stereo_chunk(frame *buf, size_t frame_count,
                             filter_state *fstate,
                             float left_gain, float right_gain)
{
    // XXX Implement me.
    filter_chunk(&buf[0][0], frame_count, 2, fstate);
    filter_chunk(&buf[0][1], frame_count, 2, fstate);
    for (size_t i = 0; i < frame_count; i++) {
        buf[i][0] *= left_gain;
        buf[i][1] *= left_gain;
    }
